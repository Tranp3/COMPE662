...
class Node(wsnlab.Node):
    ...
    def remove_tx_range(self):
        """Removes transmission range of the node.
        ...
        """
        if hasattr(self, 'tx_circle_id') and self.tx_circle_id is not None:
            self.scene.delshape(self.tx_circle_id)
            self.tx_circle_id = None
    ...
    def draw_tx_range(self):
        ...
        """
        self.tx_circle_id = self.scene.circle(self.pos[0], self.pos[1], self.tx_range, line="wsnsimpy:tx")
        #self.delayed_exec(0.2, self.scene.delshape, self.tx_circle_id)
...
class Simulator(wsnlab.Simulator):
    ...
    def __init__(self, duration, timescale=1, seed=0, terrain_size=(1000, 1000), visual=True, title=None):
        ...
        if self.visual:
            ...
            # expose the simulator instance to the plotter so the plotter
            # can query node attributes (used by click-to-inspect feature)
            try:
                # set a back-reference so Plotter can access sim.nodes
                self.tkplot.sim = self
            except Exception:
                pass

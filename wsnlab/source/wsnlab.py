...
class Node:
    ...
    def __init__(self, sim, id, pos):
        ...
        # lightweight message history used by the GUI inspector (Plotter)
        # store tuples of (time, packet) where packet is a shallow copy when possible
        self._sent_messages = []
    ...
    def can_receive(self, pck):
        ...
        if dest is None:
            self.log(f"Warning: can_receive called with None destination in packet: {pck}")
            return False
    ...
    def send(self, pck):
        ...
        """
        # record send for GUI; use shallow copy to avoid later mutation
        try:
            self._sent_messages.append((self.now, pck.copy() if isinstance(pck, dict) else pck))
        except Exception:
            try:
                self._sent_messages.append((self.now, pck))
            except Exception:
                pass
...
.
    ...
    def on_receive_check(self, pck):
        ...
        if not self.is_sleep:
            # record reception for GUI history
            try:
                self._received_messages.append((self.now, pck.copy() if isinstance(pck, dict) else pck))
            except Exception:
                try:
                    self._received_messages.append((self.now, pck))
                except Exception:
                    pass
